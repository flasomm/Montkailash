...
function wooVouUpdateRepeaterFields() {
     ...
     * @since 1.0
     */
    jQuery('.woo-vou-meta-color-select').click( function(){
      var jQuerythis = jQuery(this);
...
jQuery(document).ready(function($) {
  ...
  */
  ...
  // tab between them
	...
	/*jQuery( document ).on( 'change', 'select[name=_woo_vou_pdf_view]', function(){
    	
    	var pdf_view	= jQuery( this ).val();
    	
    	if( pdf_view == 'land' ) {
    		jQuery( '.woo-vou-meta-pdf-size-wrap' ).addClass( 'woo-vou-meta-display-none' );
    	} else {
    		jQuery( '.woo-vou-meta-pdf-size-wrap' ).removeClass( 'woo-vou-meta-display-none' );
    	}
    });*/
