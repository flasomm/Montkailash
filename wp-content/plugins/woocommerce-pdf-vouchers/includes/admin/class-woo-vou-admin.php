...
class WOO_Vou_Admin
{
	...
	public function woo_vou_admin_submenu()
	{
		...
		if (in_array($user_role, $woo_vou_vendor_role)) { // Check vendor user role
			...
			add_submenu_page(
				$main_menu_slug, __('Voucher Codes', 'woovoucher'), __('Voucher Codes', 'woovoucher'), WOO_VOU_VENDOR_LEVEL, $main_menu_slug, array(
				$this, 'woo_vou_codes_page',
							   )
			);
			...
			//voucher codes page
			$voucher_page = add_submenu_page(
				$main_menu_slug, __('Voucher Codes', 'woovoucher'), __('Voucher Codes', 'woovoucher'), WOO_VOU_VENDOR_LEVEL, 'woo-vou-codes', array(
				$this, 'woo_vou_codes_page',
							   )
			);
		}
		...
		$check_voucher_page = add_submenu_page(
			$main_menu_slug, __('Check Voucher Code', 'woovoucher'), __('Check Voucher Code', 'woovoucher'), WOO_VOU_VENDOR_LEVEL, 'woo-vou-check-voucher-code', array(
			$this, 'woo_vou_check_voucher_code_page',
						   )
		);
	}
	...
	public function woo_vou_editor_meta_box()
	{
		...
		add_meta_box(
			'woo_vou_page_voucher', __('Voucher', 'woovoucher'), array(
			$this, 'woo_vou_editor_control',
		), WOO_VOU_POST_TYPE, 'normal', 'high', 1
		);
		
		// add metabox for style options
		add_meta_box(
			'woo_vou_pdf_options', __('Voucher Options', 'woovoucher'), array(
			$this, 'woo_vou_pdf_options_page',
		), WOO_VOU_POST_TYPE, 'normal', 'high'
		);
	}
	...
	function woo_vou_get_preview_link($postid)
	{
		...
		$preview_url = add_query_arg(
			array(
				'post_type' => WOO_VOU_POST_TYPE, 'woo_vou_pdf_action' => 'preview', 'voucher_id' => $postid,
			), admin_url('edit.php')
		);
	...
	function woo_vou_add_preview_button()
	{
		...
		if (isset($_GET['post'])) {
			...
			<div id="duplicate-action"><a class="submitduplicate duplication"
										  href="<?php echo esc_url($notifyUrl); ?>"><?php _e('Copy to a new draft', 'woovoucher'); ?></a>
			</div>
			<?php
		}
	...
	public function woo_vou_order_meta_boxes()
	{
		...
		add_meta_box(
			'woo-vou-order-voucher-details', __('Voucher Details', 'woovoucher'), array(
			$this, 'woo_vou_display_voucher_data',
		), WOO_VOU_MAIN_SHOP_POST_TYPE, 'normal', 'default'
		);
	...
	public function woo_vou_user_edit_profile_fields($user)
	{
		...
		if ($user->has_cap('woo_vendor_options')) {
	...
	public function add_hooks()
	{
		...
		add_action(
			'manage_' . WOO_VOU_POST_TYPE . '_posts_custom_column', array(
			$this, 'woo_vou_manage_custom_column',
		), 10, 2
		);
