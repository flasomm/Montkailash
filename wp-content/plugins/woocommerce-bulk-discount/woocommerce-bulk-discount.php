...
if ( !class_exists( 'Woo_Bulk_Discount_Plugin_t4m' ) ) {
	...
	class Woo_Bulk_Discount_Plugin_t4m {
		...
		public function filter_subtotal_price( $price, $values ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( 'Incl. %s de remise', 'wc_bulk_discount' ), ( get_option( 'woocommerce_t4m_discount_type', '' ) == 'flat' ?  $coeff . get_woocommerce_currency_symbol() : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
		...
		public function filter_subtotal_order_price( $price, $values, $order ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( 'Incl. %s de remise', 'wc_bulk_discount' ), ( $discount_type == 'flat' ?  $coeff . get_woocommerce_currency_symbol() : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
